...
$salutation = input_post('salutation');
...
if (empty($salutation) || empty($first_name) || empty($last_name)) {
    $error = 'Kolom wajib diisi tidak boleh kosong.';
} elseif (!in_array($salutation, array_keys(salutation()))) {
...
$insert = $db->query(
    "INSERT INTO contacts SET salutation = $salutation, first_name = '$first_name', " .
        ...
        "role = '$role', enhancer = '$enhancer'"
);

if (!$insert) {
    set_flashdata('error', 'Contact gagal ditambahkan.');
} else {
    set_flashdata('success', 'Contact berhasil ditambahkan.');
}
